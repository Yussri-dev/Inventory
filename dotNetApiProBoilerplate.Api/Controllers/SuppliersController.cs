...
using Inventory.Services.Features.Suppliers.GetById;
using Inventory.Services.Features.Suppliers.Search;
    ...
    public class SuppliersController : ControllerBase
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var result = await _mediator.Send(
                new CreateSupplierCommand(request),
                cancellationToken
            );
            ...
            CancellationToken cancellationToken)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            var result = await _mediator.Send(
                new UpdateSupplierCommand(id, request),
                cancellationToken
            );
            return Ok(result);
        }
        ...
        {
            var result = await _mediator.Send(
                new SearchSuppliersQuery(query),
                cancellationToken
            );
            return Ok(result);
        }
